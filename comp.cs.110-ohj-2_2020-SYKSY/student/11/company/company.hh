...
private:
    ...
    Employee * GetId(const std::string &id)const;
    ...
    //Checks Id;
    bool checkForId(const std::string &id)const;

    //Checks if two ids are in same hierarchy
    bool checkHierarchy(const std::string &id1,const std::string &id2)const;

    //tree traversal using recursion
    IdSet recursiveSub(Employee* start,IdSet &subs, const int n)const;
};
