...
int main() {
    ...
    while ( true ) {
        ...
        while (command_to_be_executed[i]){
          ...
          command_to_be_executed[i]=toupper(c);
          i++;
        }
        //cout << command_to_be_executed<< endl;
        Command command_found;
        unsigned long j=0;
        while(command_to_be_executed!=command_found.str || j==COMMANDS.size()){
            if(j==COMMANDS.size()){
                break;
            }
            command_found=COMMANDS.at(j);
            j++;
        }
        if(j==COMMANDS.size() &&
           command_to_be_executed!=command_found.str){
            cout <<"Error: unknown command."<< endl;
            error=true;

        }
        //cout << "found: "<<command_found.str << endl;
        //cout << "size: "<<pieces.size() << endl;
        if(command_found.parameter_number==0 && pieces.size()!=1 && error==false){
            cout <<"Error: wrong number of parameters."<< endl;
            error=true;
        }
        else if(command_found.exit==true && error==false){
            cout << GREETING_AT_END << endl;
            break;
        }
        else if(command_found.parameter_number!=pieces.size()-1 && error==false){
            cout <<"Error: wrong number of parameters."<< endl;
            error=true;
        }
        if(error==false){
            double test=0;
            double test2=0;
            i=command_found.parameter_number;
            while(i!=0){
                bool operand_test=string_to_double(pieces.at(i),test);
                if(operand_test==false){
                    cout <<"Error: a non-number operand."<< endl;
                    error=true;
                    break;
                }
                else if(operand_test==true){
                    if(test2==0){
                        test2=test;
                    }
            ...
            if(error==false){
                cout <<command_found.action(test,test2)<< endl;
            }
        }
