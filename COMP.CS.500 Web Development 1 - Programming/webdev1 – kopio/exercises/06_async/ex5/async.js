async function sequentialStart(i,timeout,actors) {
...
function drawArrows(actors, timeout, drawArrow, i = 0) {
    //for (i; i<(actors.length-1)+(actors.length-1); i++) {
        if((actors.length-1)+(actors.length-1)==i) {
            console.log(actors.length+1);
            ...
            (async () => {
                for (i; i<(actors.length-1)+(actors.length-1); i++) {
                        ...
                        const res = await sequentialStart(i,timeout,actors)

                }
                
                //const res = await Promise.resolve("Go, IIFE, GO!");
                //console.log(res); // Go, IIFE, GO!
            })()
            //for (i; i<(actors.length-1)+(actors.length-1); i++) {
                //myFunction(actors, timeout, drawArrow, i)
                 //.then()
                  //.then(drawArrows(actors, timeout, drawArrow, i))
                 //.catch(console.error);
            //}
