...
public:
    ...
    // Estimate of performance: Constant O(18-34)? on average, worst case linear in the size of the container.
    // Short rationale for estimate:
    // insert() and find() are both constant on average, but they can be worse.
...
private:
    ...
    //Checks that WayId is in the datastucture
    ...
    std::vector<std::tuple<Coord, WayID, Distance> > dfs(Node *source);
};
